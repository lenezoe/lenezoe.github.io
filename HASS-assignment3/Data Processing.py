...
df_new = df_new.groupby(['year', 'flat_type', 'town','remain_leaseyears','rounded_lease']).agg({'resale_price': 'mean'}).reset_index()
df_new['resale_price'] = df_new['resale_price'].round(decimals=0)
df_new.head()

# Save the DataFrame as a CSV file
df_new.to_csv('meanflatprices_2022.csv', index=False)
...
### MATURE TOWN
# Aggregate the DataFrame by year, flat_type, and remain_leaseyears columns
df_mature = df_mature.groupby(['year', 'flat_type', 'town','remain_leaseyears','rounded_lease']).agg({'resale_price': 'mean'}).reset_index()
df_mature['resale_price'] = df_mature['resale_price'].round(decimals=0)
df_mature.head()

# Save the DataFrame as a CSV file
df_mature.to_csv('meanflatprices_mature2022.csv', index=False)

### nonmature TOWN
# Aggregate the DataFrame by year, flat_type, and remain_leaseyears columns
df_nonmature = df_nonmature.groupby(['year', 'flat_type', 'town','remain_leaseyears','rounded_lease']).agg({'resale_price': 'mean'}).reset_index()
df_nonmature['resale_price'] = df_nonmature['resale_price'].round(decimals=0)
df_nonmature.head()

# Save the DataFrame as a CSV file
df_nonmature.to_csv('meanflatprices_nonmature2022.csv', index=False)
